  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className={`text-sm ${textMuted} mt-1`}>Manage your account settings and preferences</p>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className={`rounded-2xl border ${surface} p-6`}>
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 className={`text-xl font-bold ${textPrimary}`}>{profile.name}</h2>
              <p className={`text-sm ${textSecondary}`}>{profile.role}</p>
              <p className={`text-xs ${textMuted} mt-1`}>{profile.department}</p>
            </div>

            <div className="space-y-4">
              <ProfileItem icon="mail" label="Email" value={profile.email} isDark={isDark} renderIcon={renderIcon} />
              <ProfileItem icon="map" label="Location" value={profile.location} isDark={isDark} renderIcon={renderIcon} />
              <ProfileItem icon="user" label="Role" value={profile.role} isDark={isDark} renderIcon={renderIcon} />
            </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className={`rounded-2xl border ${surface} p-6`}>
            <div className="flex items-center justify-between mb-4">
              <h3 className={`text-lg font-semibold ${textPrimary}`}>Bio</h3>
              {isEditing && (
                <button
                  onClick={handleSave}
            ...
            {isEditing ? (
              ...
              <p className={`text-sm ${textSecondary} whitespace-pre-line`}>{profile.bio}</p>
            )}
          </div>

          {/* Notification Preferences */}
          <div className={`rounded-2xl border ${surface} p-6`}>
            <h3 className={`text-lg font-semibold mb-4 ${textPrimary}`}>Notification Preferences</h3>
            <div className="space-y-4">
              {[
                { key: 'email', label: 'Email notifications', description: 'Receive email alerts for important updates' },
                { key: 'tickets', label: 'New tickets', description: 'When a new ticket is assigned to you' },
                { key: 'replies', label: 'Replies', description: 'When someone replies to your tickets' },
                { key: 'resolved', label: 'Resolved tickets', description: 'When tickets are marked as resolved' }
              ].map((item) => (
                <div key={item.key} className="flex items-start justify-between">
                  <div>
                    <h4 className={`font-medium ${textPrimary}`}>{item.label}</h4>
                    <p className={`text-sm ${textMuted}`}>{item.description}</p>
                  </div>
                  <button
                    onClick={() => setNotifications({ ...notifications, [item.key]: !notifications[item.key] })}
                    className={`relative w-12 h-6 rounded-full transition-colors ${
                      notifications[item.key] ? 'bg-indigo-500' : isDark ? 'bg-slate-700' : 'bg-gray-300'
                    }`}
                  >
                    <div
          ...
          {/* Security */}
          <div className={`rounded-2xl border ${surface} p-6`}>
            <h3 className={`text-lg font-semibold mb-4 ${textPrimary}`}>Security</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className={`font-medium ${textPrimary}`}>Password</h4>
                  <p className={`text-sm ${textMuted}`}>Last changed 2 months ago</p>
                </div>
      ...
      {/* Edit Modal (Optional - inline for simplicity) */}
      {isEditing && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className={`w-full max-w-2xl rounded-2xl ${surface} p-6`}>
            ...
            <div className="space-y-4">
              <InputField label="Full Name" value={formData.name} onChange={(v) => setFormData({...formData, name: v})} isDark={isDark} />
              <InputField label="Email" value={formData.email} onChange={(v) => setFormData({...formData, email: v})} isDark={isDark} />
              <InputField label="Location" value={formData.location} onChange={(v) => setFormData({...formData, location: v})} isDark={isDark} />
              <TextareaField label="Bio" value={formData.bio} onChange={(v) => setFormData({...formData, bio: v})} isDark={isDark} />
            </div>
